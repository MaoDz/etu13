...
int main(int argc, char ** argv)
{
  ...
  int leave = 0;
    ...
    SDL_Surface *deck[13],*objet[8],*gobutton,*connectbutton, *score;
    ...
    SDL_Texture *texture_deck[13],*texture_gobutton,*texture_connectbutton,*texture_objet[8],*texture_score;
   ...
   synchro=0; // initialisation de la variable synchro
   // Fait une seule fois dans le programme
	...
	if (SDL_PollEvent(&event))
	{
        ...
        if(gId == id)
          ...
          goEnabled = 0;
			...
			case 'V':
				// RAJOUTER DU CODE ICI
        sscanf(gbuffer+2, "%d %d %d", &x, &y, &temp);
        if(!(temp==100 && tableCartes[x][y]!=-1))
          tableCartes[x][y]=temp;
				break;

      // Message 's' : la partie est finie
      case 's':
        quit = 1;
        leave = 1;
        break;
  ...
  if(leave == 1)
  {
    if(gId == id)
    {
      score = IMG_Load("images/Winner.png");
      texture_score = SDL_CreateTextureFromSurface(renderer, score);
      SDL_Rect dstrect = { 300, 500, 250, 250 };
      SDL_RenderCopy(renderer, texture_score, NULL, &dstrect);
    }
    else
    {
      score = IMG_Load("images/Loser.png");
      texture_score = SDL_CreateTextureFromSurface(renderer, score);
      SDL_Rect dstrect = { 300, 500, 250, 250 };
      SDL_RenderCopy(renderer, texture_score, NULL, &dstrect);
    }
