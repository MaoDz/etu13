...
int main(int argc, char *argv[])
{
	...
	{
    int coupable, objetSel, joueurSel;
		switch (buffer[0])
		{
      case 'G':
				// RAJOUTER DU CODE ICI
        sscanf(buffer+2,"%d %d", &id, &coupable);
        if (coupable==deck[12]){
          sprintf(reply, "|Bravo %s a gagné, cheh les autres\n", tcpClients[id].name);
          broadcastMessage(reply);
          stop = 1;
      ...
      case 'O':
				// RAJOUTER DU CODE ICI
        sscanf(buffer+2,"%d %d", &id, &objetSel);
        for(int i = 0; i < 4; i++)
        {
          ...
          broadcastMessage(reply);
			...
			case 'S':
				// RAJOUTER DU CODE ICI
        sscanf(buffer+2, "%d %d %d",&id, &joueurSel, &objetSel);
        sprintf(reply, "V %d %d %d", joueurSel, objetSel,
